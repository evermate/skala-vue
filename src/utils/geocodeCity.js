...
// GeoNames feature_code — PPLC(수도)/PPLA~PPLA5(각급 행정구역 소재지)는 행정적으로 유의미한
// 도시/읍. population이 없는 케이스(예: 여수는 PPLA2인데도 population이 비어 있음)가 있어서
// population 유무만으론 못 거르고, 이 코드도 같이 봐야 "이름만 같은 동네"를 걸러낼 수 있다.
...
// 긴 접미사(특별자치시 등)부터 검사해야 "시"만 잘못 떼는 걸 막을 수 있다.
...
export async function geocodeCity(name) {
  ...
  // 원본 쿼리는 실패를 그대로 전파(네트워크 에러 등은 상위에서 안내 문구로 처리),
  ...
  // 북한 동명 마을)는 목록에서 아예 뺀다. 전부 다 유의미하지 않은 경우(정말 작은 동네만
